...
- [x] 安装并配置 `expo-image-picker`；资料页新增“从照片库选择头像”和“拍摄头像”，支持取消选择。
- [ ] 在真机验证首次授权、已授权、拒绝授权、在系统设置重新授权四种流程；拒绝时显示“打开系统设置”引导，不循环请求权限。
- [x] 使用 `expo-image-manipulator` 将头像居中裁剪并压缩为 512 × 512 JPEG；当前以本地预览作为 mock，不新增上传接口。
- [x] 安装 `expo-document-picker` 与 `expo-file-system`，实现“选择订单附件”练习区；展示文件名、MIME 类型、大小与取消状态，不读取或记录文件内容。
- [x] 使用 `expo-sharing` 分享已选择的附件；在不支持分享的环境显示不可用状态。
- [ ] 在真机完成上述流程；自动化质量与后端回归已通过，真机验收清单见 `plans/2026-09-14-week-4-media-files-record.md`。
